...
地狱通过 `nether-mapping` 投影主世界区域。默认世界为 `world_nether`，比例为 8；地狱坐标乘以 8 后对应主世界坐标。实现中应预先把多边形坐标除以比例生成地狱多边形，不能在每次玩家移动时重新创建。地狱同样返回 `san_cheng`、`huadu`、`Hutong` 或 `outskirts`。

除主世界和映射地狱外，其他世界返回合成世界区域：

- ID：`world:<Bukkit 世界名>`；
- 名称：优先使用 `world-display-names`，否则使用 Bukkit 世界名；
- 默认 `world_the_end → 末地`；未配置名称的自定义世界使用 Bukkit 世界名。

主世界和地狱的行政区外都返回配置项 `main-world-outside-area` 定义的远郊区域，默认 `outskirts / 远郊`。其他世界返回合成 `Area`。对已加载的有效世界，API 因此始终能返回明确区域。
...
- 是否保持主世界与地狱映射行政区/远郊、其他世界区域的返回契约？
- 是否保持末地飞机限制的所有入口？
